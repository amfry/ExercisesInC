...
struct Sigfunc *Signal(int signo, struct Sigfunc *func)	/* for our signal() function */
{
  Sigfunc *struct sigfunc;

  if ( (struct sigfunc = signal(signo, func)) == SIG_ERR)
    err_sys("signal error");
  return(struct sigfunc);
}
