...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        int connect_d = accept(listener_d, (struct stockaddr *) &client_addr, &address_size);
        if(connect_d == 1)
        ...
        if (/fork){
          ...
          ! = -1){
            ...
            if(strncaseamp("Whose there?",buf,10))
              say(connect_d, "You should day 'Who's there'\r\n");
            else{
              say(connect_d, "Oscar silly questions, you get a silly answer\r\n")              }
            }
          }
    }
    return 0;
